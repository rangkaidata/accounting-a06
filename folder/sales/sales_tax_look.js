...
function SalesTaxLook(indek_parent,cari){
  
  function show(){
    const tiket=JSON.parse(JSON.stringify(bingkai[indek_parent]));
    tiket.parent=indek_parent;
    tiket.modul='sales_taxes';
    tiket.menu.name="Sales Taxes"
  ...
  function dataShow(indek){
    ...
    if (paket.err.id===0){
      for (var x in paket.data) {
        ...
        +'<td align="center">'+paket.data[x].sales_tax_rate+'%</td>'
        +'<td align="center">'
  ...
  function select(indek,a){
    ...
    switch(bingkai[indek_parent].menu.id){
      case "customers":
        Customers.setSalesTax(indek_parent,data[a]);
        break;

      default:
        alert('['+bingkai[indek_parent].menu.id+']'
          +' undefined in [sales_tax_look.js]');
    }
    ui.CLOSE_POP(indek);
  }
