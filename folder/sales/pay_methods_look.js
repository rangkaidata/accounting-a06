 ...
 * edit: sep-19, 21:34, tue-2023;
 */ 
 
'use strict';

function PayMethodLook(indek_parent,cari){
  function show(){
    const tiket=JSON.parse(JSON.stringify(bingkai[indek_parent]));
    tiket.parent=indek_parent;
    tiket.modul='pay_methods';
    tiket.menu.name="Pay Methods"
  ...
  function dataShow(indek){
    ...
    if (paket.err.id===0){
      for (var x in paket.data) {
        ...
        +'<td align="left">'+paket.data[x].pay_method_id+'</td>'
        +'<td align="left">'+xHTML(paket.data[x].pay_method_name)+'</td>'
        +'<td align="center">'
          +'<button type="button" '
  ...
  function select(indek,a){
    ...
    switch(bingkai[indek_parent].menu.id){
      case "customer_defaults":
        CustomerDefaults.setPayMethod(indek_parent,data[a]);
        break;

      default:
        alert('['+bingkai[indek_parent].menu.id+'] '
          +' undefined in [pay_methods_look.js]');        
    }
    ui.CLOSE_POP(indek);
  }
