...
Boms.lookUp=(indek,id_kolom,id_baris)=>{
  bingkai[indek].id_kolom=id_kolom;
  bingkai[indek].id_baris=id_baris;

  objPop=new BomLook(indek);
  objPop.show();
}

function BomLook(indek_parent){
  
  function show(){
    const tiket=JSON.parse(JSON.stringify(bingkai[indek_parent]));
    tiket.parent=indek_parent;
    tiket.modul='boms';
    tiket.menu.name="Bill of Material"
  ...
  function select(indek,a){
    ...
    switch(bingkai[indek_parent].menu.id){
      case "builds":
        Builds.setBom(indek_parent,data[a]);
        break;
      case "unbuilds":
        Unbuilds.setBom(indek_parent,data[a]);
        break;

      default:
        alert('['+bingkai[indek_parent].menu.id
          +'] undefined in boms_look.js.');
    }
    ui.CLOSE_POP(indek);
  }
