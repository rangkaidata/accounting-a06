...
function ShipMethodLook(indek_parent,cari){
  ...
  function dataShow(indek){
    ...
    if (paket.err.id===0){
      for (var x in paket.data) {
        ...
        +'<td align="left">'+xHTML(paket.data[x].ship_name)+'</td>'
        +'<td align="center">'
          +'<button type="button" '
  ...
  function select(indek,a){
    ...
    switch(bingkai[indek_parent].menu.id){
      case "vendors":
        Vendors.setShip(indek_parent,data[a]);
        break;
      case "customers":
        Customers.setShip(indek_parent,data[a]);
        break;
      default:
        alert('['+bingkai[indek_parent].menu.id
          +'] undefined in [ship_method_look.js]');
