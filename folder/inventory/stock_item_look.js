...
StockItems.lookUp=(indek,id_kolom,nama_kolom)=>{
  bingkai[indek].id_kolom=id_kolom;
  bingkai[indek].nama_kolom=nama_kolom;

  objPop=new StockItemLook(indek);
  objPop.show();
}

StockItems.getOne=(indek,item_id,callBack)=>{
  db3.query(indek,'stock_items/read_one',{
    "item_id":item_id
  },(paket)=>{
...
function StockItemLook(indek_parent){
  function show(){
    const tiket=JSON.parse(JSON.stringify(bingkai[indek_parent]));
    tiket.parent=indek_parent;
    tiket.modul='stock_items';
    tiket.menu.name="Stock Item Lists"
  ...
  function select(indek,a){
    ...
    switch(bingkai[indek_parent].menu.id){
      case "boms":
        Boms.setStockItem(indek_parent,data[a]);
        break;
      case "moves":
        Moves.setStockItem(indek_parent,data[a]);
        break;
      case "adjustments":
        Adjustments.setStockItem(indek_parent,data[a]);
        break;

      default:
        alert('['+bingkai[indek_parent].menu.id+']'
          +' undefined in [stock_item_look.js]');
    }
    ui.CLOSE_POP(indek);
